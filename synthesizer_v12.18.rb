...
# Global variables for instrument management
$instrument_ids = []
$instrument_data = []
$pending_instruments = []
...
# Configuration
$sample_base_path = "C:/Users/marli_/Documents/instruments"
...
define :play_single_instrument do |id, inst|
  ...
  # Calculate stereo balance and overall volume
  ...
  # Choose ONE melody pattern for this instrument's lifetime
  ...
  in_thread do
    ...
    while true
      ...
      # Get rpitch value from the chosen melody pattern
      rpitch_value = melody_pattern[note_index]
      
      # Convert rpitch value to rate (semitone calculation)
      rate_value = 2.0 ** (rpitch_value / 12.0)
      
      # Play sample with stereo panning based on left/right volumes
      sample sample_path,
        rate: rate_value,
        amp: total_volume * 0.8,
        pan: stereo_balance
      
      # Advance melody (cycle through the fixed pattern)
      note_index = (note_index + 1) % melody_pattern.length
      
      # When we reach the end of the pattern, we could change chord progression
      # (but NOT the melody pattern)
...
# OSC input handlers - UPDATED to receive pitch, timbre, tempo, left_volume, right_volume
